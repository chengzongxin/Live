...
{
    unsigned int r, g, b;
    ColorModel *rgb = [self ColorWithRGBHexString:color withAlpha:alpha];
    r = rgb.R;
    g = rgb.G;
    b = rgb.B;
    alpha = rgb.alpha;
...
+ (UIColor *)randomColor
{
    UIColor *color = [UIColor colorWithRed:((arc4random()%256) / 255.0f) green:((arc4random()%256) / 255.0f) blue:((arc4random()%256) / 255.0f) alpha:1];
    return color;
}
